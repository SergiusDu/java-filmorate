...
public class JdbcFilmRepository implements FilmRepository {
  ...
  public Film save(CreateFilmCommand command) {
    ...
    long filmId = simpleJdbcInsert.executeAndReturnKey(params)
                                  .longValue();
    updateFilmGenres(filmId,
                     command.genres());

    return filmFactory.create(filmId,
                              command);
  }
  ...
  public Optional<Film> findById(long id) {
    ...
    List<Film> films = mapRowsToFilms(jdbcTemplate.queryForList(sql,
                                                                id));
    return films.stream()
                .findFirst();
  }
  ...
  public List<Film> getByIds(Set<Long> ids) {
    ...
    String inSql = String.join(",",
                               Collections.nCopies(ids.size(),
                                                   "?"));
    String sql = String.format(BASE_FILM_QUERY + " WHERE f.film_id IN (%s)",
                               inSql);
    return mapRowsToFilms(jdbcTemplate.queryForList(sql,
                                                    ids.toArray()));
  }
    ...
    return rows.stream()
               .map(row -> {
                 ...
                 return new Film(filmId,
                                 ...
                                 genres,
                                 new Mpa((Long) row.get("mpa_id"),
                                         (String) row.get("mpa_name")));
               })
    ...
    jdbcTemplate.query(sql,
                       rs -> {
                         ...
                         Genre genre = new Genre(rs.getLong("genre_id"),
                                                 rs.getString("name"));
                         genresByFilmId.computeIfAbsent(filmId,
                                                        k -> new HashSet<>())
                                       .add(genre);
                       },
                       filmIds.toArray());
  ...
  private void updateFilmGenres(long filmId, Set<Genre> genres) {
    jdbcTemplate.update("DELETE FROM film_genres WHERE film_id = ?",
                        filmId);
    if (genres != null && !genres.isEmpty()) {
